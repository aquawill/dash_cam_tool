...
1. 想要把每天的行車紀錄器影片都備份下來，但是影片太龐大，硬碟快被塞滿了，就算上傳到雲端，也要非常久的時間。
2. 影片跟軌跡是分開的檔案，這樣一定要用原廠軟體才可以查到影片當下的拍攝位置。可是原廠軟體通常也不是那麼好用，如果影片數量多的話更是麻煩。
3. 如果要把影片上傳到雲端，就沒辦法結合GPS的資料。
4. 行車記錄器的GPS軌跡原始檔不知道要用什麼軟體開啟。
5. 不想多花錢買額外的儲存設備，但又希望可以有一個簡單快速備份的方法。
6. 希望可以隨時隨地查閱之前的行車紀錄器存檔，不用一定要回家開電腦。

因為有了以上的困擾，所以我開發了一個小小工具軟體，希望能夠節省大家的精力。這個小軟體主要的功能有：
1. 把行車記錄器的影片，以一秒擷取一格或半秒擷取一格的方式存成JPG圖檔。
2. 用影片拍攝的時間與GPS軌跡的時間來校正JPG圖檔的拍攝時間。
3. 把GPS的位置資料寫入JPG圖檔。
